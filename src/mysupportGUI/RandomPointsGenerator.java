...
{
	return Math.min(Math.min(Math.min(Math.sqrt(
			  Math.pow((double) (x - maxWidth / 2), 2.0D) + Math.pow((double) (y - maxHeight / 2), 2.0D)),
	                                  Math.sqrt(Math.pow((double) x - 2.5D * (double) maxWidth / 6.0D, 2.0D) +
	                                            Math.pow((double) (y - 2 * maxHeight / 6), 2.0D))),
	                         Math.min(Math.sqrt(Math.pow((double) (x - 4 * maxWidth / 6), 2.0D) +
	                                            Math.pow((double) (y - 2 * maxHeight / 6), 2.0D)),
			...
			for (int i = 0; i < numberOfPoints; ++i)
			{
				...
				do
				{
					...
					while (distanceToCenter(x, y) >= (double) radius * 1.4D &&
					       ...
					       (maxHeight / 9 >= x || x >= 4 * maxHeight / 5 || maxHeight / 9 >= y || y >= 7 * maxHeight / 9 ||
